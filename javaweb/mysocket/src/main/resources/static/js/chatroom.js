    ...
    function deleteFriend() {
        ...
        var fId;
        var tp;
        if ($("#toUserId").val().length != 0) {
            u_or_g = "是否删除好友："+ $(".headName").text() + " ？";
            fId = $("#toUserId").val();
            tp = 1;
        } else {
            u_or_g = "是否退出群聊："+ $(".headName").text() + " ？";
            fId = $("#toGroupId").val();
            tp = 2;
        }
        var confm = confirm(u_or_g);
        if(confm == false) {
            return false;
        }
        $.ajax({
            type : 'POST',
            url : 'chatroom/deleteFri_Grp',
            dataType: 'json',
            data : {
                userId : userId,
                fId : fId,
                ...
                if (data.status == 200) {
                    ...
                    if( tp == 1 ) {
                        $('.conLeft').find('span.hidden-userId').each(function(){
                            if (this.innerHTML == fId) {
                                $(this).parent(".liRight").parent("li").remove();
                            }
                        })
                    } else {
    ...
    function alter_avatar() {
        if (socket.readyState == WebSocket.CLOSED) {
            return false;
        }
        var th=$("#avatarUrl")[0].src
        $("#alter_tar")[0].src=th;
    }
    ...
    // 添加操作 ,tp表示加群 还是加好友
    function toadd_friend(fId,tp){
        $.ajax({
            type : 'POST',
            url : 'chatroom/toadd_friend',
            dataType: 'json',
            data: {
                userId : userId,
                fId : fId,
            ...
            success: function(data) {
                if (data.status == 200) {
                    ...
                    $("#add").css("background-color","#cec8c8");
                    $("#cdd").append('<span style="background-color: #c86dee">'+ info.warn +'</span>')
                } else {
                    alert(data.msg);
                }
            }
    ...
    function addFriend() {
        ...
        $.ajax({
            ...
            async : true,
            success: function(data) {
                var fuser = data.data.user;
                var fgroup = data.data.group;
                if (fuser == null && fgroup == null) {
                    addHTML +='<div>' +
                        '<span>'+ data.data.msg +'</span>' +
                        '</div>'
                } else if(fgroup == null){
                    addHTML += '<div id="cdd" style = "border: #a6e1ec 1px dashed;">' +
                        '<div style="margin-top: 10px;margin-left: 20px;" class="liLeft">&nbsp;&nbsp;&nbsp;头&nbsp;像&nbsp;&nbsp;：<img src="' + fuser.avatarUrl + '"></div>' +
                        '<div style="margin-left: 20px;margin-bottom: 10px" class="liRight">' +
                        '<span class="intername">&nbsp; &nbsp;I&nbsp;&nbsp;D&nbsp;&nbsp;：' + fuser.userId + '</span>' +
                        '<span class="intername">用户名&nbsp;：' + fuser.username + '</span>' +
                        '<button id="add" onclick="toadd_friend('+fuser.userId+',1)" style="margin-left: 20px" class="btn btn-primary">添加</button>' +
                        '</div>' +
                    '</div>';
                } else {
                    addHTML += '<div id="cdd" style = "border: #a6e1ec 1px dashed;">' +
                        '<div style="margin-top: 10px;margin-left: 20px;" class="liLeft">&nbsp;&nbsp;&nbsp;群&nbsp;头&nbsp;&nbsp;：<img src="' + fgroup.groupAvatarUrl + '"></div>' +
                        '<div style="margin-left: 20px;margin-bottom: 10px" class="liRight">' +
                        '<span class="intername">&nbsp; &nbsp;I&nbsp;&nbsp;D&nbsp;&nbsp;：' + fgroup.groupId + '</span>' +
                        '<span class="intername">群名称&nbsp;：' + fgroup.groupName + '</span>' +
                        '<button id="add" onclick="toadd_friend('+fgroup.groupId+',2)" style="margin-left: 20px" class="btn btn-primary">添加</button>' +
                        '</div>' +
                        '</div>';
                }

                $("#content_div").append(addHTML);
            }
        });
    }
    ...
    var ws = {
        ...
        singleReceive: function(data) {
            ...
            var fromAvatarUrl = data.fromAvatarUrl;
            var $receiveLi;
            $('.conLeft').find('span.hidden-userId').each(function(){
        ...
        groupReceive: function(data) {
            ...
            var answer='';
            if (content.search("<img") != -1) { // 若是图片，
                answer += '<li>' +
                    '<div class="answerHead"><span class="answerName">'+fromUserName+'</span><img src="' + fromAvatarUrl + '"/></div>' +
                    '<div class="answers-group_emo">'+ content +'</div>' +
                    '</li>';
            } else {
                answer += '<li>' +
                    '<div class="answerHead"><span class="answerName">'+fromUserName+'</span><img src="' + fromAvatarUrl + '"/></div>' +
                    '<div class="answers-group">'+ content +'</div>' +
                    '</li>';
            }
        ...
        fileMsgGroupRecieve: function(data) {
            ...
            '<li class="Rfl">'+
                '<div class="answerHead"><span class="answerName">'+ fromUserName +'</span><img src="' + fromAvatarUrl + '"/></div>' +
                '<div class="receive-file-shown-group">' +
                    '<div class="media">' +
                        '<a href="' + fileUrl + '" class="media-right">' +
                            '<i class="glyphicon glyphicon-file" style="font-size:28pt;"></i>' +
                        '</a>' +
    ...
    $(".avatarImg").fileinput({
        ...
        uploadUrl:"chatroom/avatar_upload",
    ...
    //异步上传错误返回结果处理
    $('.avatarImg').on('fileerror', function(event, data, msg) {
        console.log("fileerror");
        console.log(data);
    });
    //异步上传返回结果处理
    $(".avatarImg").on("fileuploaded", function(event, data, previewId, index) {

        // 1. 上传成功1.5秒后自动关闭上传模态框
        console.log("fileuploaded");
        setTimeout(function() {
            $('#avatar-upload-cancel').trigger('click');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status == 200) {
                    $("#avatarUrl")[0].src=fileUrl;
                    $("#alter_tar")[0].src=fileUrl;
                } else {
                    alert(data.msg);
                }
            }
    ...
    $(".myfile").on("fileuploaded", function(event, data, previewId, index) {
        ...
        var fileHtml =
            '<li class="Sfl">'+
            '<div class="nesHead_file"><img src="' + avatarUrl + '"/></div>' +
